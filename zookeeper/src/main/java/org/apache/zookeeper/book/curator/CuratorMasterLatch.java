...
public class CuratorMasterLatch implements Closeable, LeaderLatchListener {
			...
			try {
				...
				case CHILDREN:
					if (event.getPath().contains("/assign")) {
						LOG.info("Succesfully got a list of assignments: " + event.getChildren().size() + " tasks");
						/*
	...
	public static void main(String[] args) {
		CuratorMasterLatch master = null;
		try {
			master = new CuratorMasterLatch(args[0], args[1], new ExponentialBackoffRetry(1000, 5));
			master.startZK();
			master.bootstrap();
			master.runForMaster();
		} catch (Exception e) {
			...
			try {
				...
				LOG.error("Exception when close CuratorMasterLatch: ", e);
			}
		}
	}
}