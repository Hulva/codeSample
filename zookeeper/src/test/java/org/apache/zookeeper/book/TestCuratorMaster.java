...
public class TestCuratorMaster extends BaseTestCase {
	...
	public void testTaskAssignment() throws Exception {
		...
		while (!c.isConnected() && !w1.isConnected() && !w2.isConnected() && !w3.isConnected()) {
			Thread.sleep(100);
		}
	...
	public void electSingleMaster() throws Exception {
		...
		boolean singleMaster = ((m.isLeader() && !bm.isLeader()) || (!m.isLeader() && bm.isLeader()));
	...
	public void testTaskAssignmentLatch() throws Exception {
		LOG.info("Starting master (taskAssignment)");
		CuratorMasterLatch m = new CuratorMasterLatch("M1", "localhost:" + port, new ExponentialBackoffRetry(1000, 5));
		m.startZK();
