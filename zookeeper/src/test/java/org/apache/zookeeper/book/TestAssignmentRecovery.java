...
public class TestAssignmentRecovery extends BaseTestCase {
	...
	public void testRecovery() {
		try {
			ZooKeeper zk = new ZooKeeper("c7001.hulva.luva.h:" + port, 10000, new Watcher() {
			...
			/*
			 * The number of recovered tasks should be 2 because there is a
			 * single active worker, one task has been assigned to an absent
			 * worker, and one task hasn't been assigned at all. The last two
			 * need to be assigned, and consequently they are part of the list
			 * of recovered tasks.
			 *
			 * Note that recovery here refers to tasks that a new master needs
			 * to reassign when failing over. It is not related to the recovery
			 * of the tasks themselves as it could happen if a worker crashes
			 * before completing a task. Recovering a task is out of the scope
			 * of this example and it is application specific.
			 * 
			 */
	...
	public void testRecoveryStatus() {
		try {
			...
			/*
			 * The two tasks that have been assigned to the absent worker have
			 * completed, so there is no need to reassign them.
			 */
			zk.create("/tasks", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			zk.create("/workers", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			zk.create("/assign", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
	...
	public void testRecoveryMissingTaskFromTasks() {
		try {
			...
			/*
			 * Task has been assigned to a worker that has crashed and it is not
			 * in the list of tasks any longer.
			 */
			zk.create("/tasks", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			zk.create("/workers", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			zk.create("/assign", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
