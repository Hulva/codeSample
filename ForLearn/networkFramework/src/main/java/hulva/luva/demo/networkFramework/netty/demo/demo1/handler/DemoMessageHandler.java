...
import hulva.luva.demo.networkFramework.common.CommandResponse;
import hulva.luva.demo.networkFramework.common.Message;
import hulva.luva.demo.networkFramework.common.MessageConstants;
...
public class DemoMessageHandler implements WebSocketMessageHandler {
  ...
  public DemoMessageHandler(ChannelGroup channels) {
    ...
    ExecutorService toClientExecutor = Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setDaemon(true).setNameFormat("to-client-%d").build());
    ...
    ExecutorService toKafkaExecutor = Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setDaemon(true).setNameFormat("to-kafka-%d").build());
  ...
  public String handleMessage(ChannelHandlerContext ctx, String frameText) {
    ...
    if (message.getType().equals("TickerCommand") || message.getType().equals("UpDownCommand") || message.getType().equals("WeatherCommand")) {
  ...
  /**
   * This class waits for command requests from the client request queue, then sends them to Kafka
   *
   * @author jwb
   */
      ...
      try {
        ...
        while (keepRunning.get()) {
          while ((request = requestQueue.poll()) != null) {
            ...
            if (topic != null) {
              producer.send(new ProducerRecord<>(topic, request), (metadata, e) -> {
                if (e != null) {
